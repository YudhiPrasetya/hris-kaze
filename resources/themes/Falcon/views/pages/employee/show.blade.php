		...
		<x-bootstrap::column>
			<x-bootstrap::card>
				<x-bootstrap::card.header>
					<x-bootstrap::row class="align-items-baseline justify-content-between">
						...
						<x-bootstrap::column breakpoint="EXTRA_SMALL|6;SMALL|auto" class="d-flex align-items-baseline">
							<a href="{{ route('employee.edit', ['employee' => $model->id]) }}"
								class="btn-falcon-success btn mr-1" role="button"><i
									class="fad fa-pencil-alt mr-2"></i>Edit</a>
							<button class="btn-falcon-danger btn" type="button"
							...
							<form id="delete-form-{{ $model->id }}"
								action="{{ route('employee.destroy', ['employee' => $model->id]) }}" method="POST"
								style="display: none;">
				...
				<x-bootstrap::card.body class="bg-light">
					<x-bootstrap::row>
						<x-bootstrap::column breakpoint="EXTRA_SMALL;MEDIUM|2"
							class="d-flex flex-column align-items-center justify-content-center">
							<div class="avatar w-90 mb-3">
								<figure class="figure w-100">
									<img id="targetPreview"
										src="@if(!$model->profile_photo_path->isEmpty()){!! $model->profile_photo_path->toString() !!}@else{{ route('image-placeholder', ['size' => '512', 'bgColor' => '#EEF0F2', 'textColor' => '#ffffff']) }}@endif"
										class="figure-img img-fluid rounded-circle img-thumbnail w-100 transition-all"
										alt="Profile Picture" />
								</figure>
							</div>
						</x-bootstrap::column>
								...
								<div class="fancy-tab-contents mt-3">
									<div class="tab-content fancy-tab-content active">
										...
										<div class="form-group">
											...
											<span id="position"
												class="form-control-plaintext text-1000 fs-0">{{ $model->jobTitle()->first()->name ?? "-"}}</span>
										</div>
										<div class="form-group">
											<h6 class="text-600 control-label mb-1">Position</h6>
											<span id="jobtitle"
												class="form-control-plaintext text-1000 fs-0">{{ $model->position()->first()->name ?? "-"}}</span>
										</div>
										@php
										...
										<div class="form-group">
											...
											<span id="effective_since"
												class="form-control-plaintext text-1000 fs-0">{{ $model->effective_since->format('l, d F Y') }}
												&mdash; {{ $year }} years, {{ $month }} months, {{ $day }} days</span>
										</div>
										<div class="form-group">
											<h6 class="text-600 control-label mb-1">Remaining Leave Quota</h6>
											<span id="leaveAllowance" class="form-control-plaintext text-1000 fs-0">
												{{-- {{$model->leave_allowance == null ? '0' : $model->leave_allowance}}
												--}}
												{{ $payroll->remainingLeaveQuota }}
											</span>
										</div>
										<div class="form-group">
										...
										<x-bootstrap::row>
											<x-bootstrap::column breakpoint="MEDIUM|6">
												<x-bootstrap::media variant="primary" class="my-4"
													icon="fad fa-money-check-alt" title="Salary"
													subtitle="Employee salary." />
												<x-bootstrap::column class="px-0">
													<x-bootstrap::media variant="primary" class="mt-4"
														title="<small>Earnings</small>" />
												</x-bootstrap::column>
												...
												<div class="form-group row mb-0">
													...
													<div class="col-sm-7 col-form-label">
														{!! $moneyFormat($model->functional_allowance, $model->currencyCode()) !!}
													</div>
												</div>
												<div class="form-group row mb-0">
													<label class="col-sm-5 col-form-label">Transport Allowance</label>
													<div class="col-sm-7 col-form-label">
														{!! $moneyFormat($model->transport_allowance, $model->currencyCode()) !!}
													</div>
												</div>
												<div class="form-group row mb-0">
													<label class="col-sm-5 col-form-label">Meal Allowances</label>
													<div class="col-sm-7 col-form-label">
														{!! $moneyFormat($model->meal_allowances, $model->currencyCode()) !!}
													</div>
												</div>
												<div class="form-group row mb-0">
													<label class="col-sm-5 col-form-label">Eid Allowance (THR)</label>
													<div class="col-sm-7 col-form-label">
														{!! $moneyFormat($model->eid_allowance, $model->currencyCode()) !!}
													</div>
												</div>
												<div class="form-group row mb-0">
													<label class="col-sm-5 col-form-label">Other Allowances</label>
													<div class="col-sm-7 col-form-label">
														{!! $moneyFormat($model->other_allowance, $model->currencyCode()) !!}
													</div>
												</div>
												<div class="form-group row mb-0">
													<label class="col-sm-5 col-form-label">
														Overtime<br />
														<small class="text-600">Total overtimes
															{{ $payroll->employee->presences->overtimeDays }} day(s)</small>
													</label>
													<div class="col-sm-7 col-form-label">
														{!! $moneyFormat($payroll->result->earnings->overtime, $model->currencyCode()) !!}
													</div>
												</div>
												<div class="form-group row mb-0">
													<label class="col-sm-5 col-form-label">Attendance
														Premium<br /><small>Premi kehadiran</small></label>
													<div class="col-sm-7 col-form-label">
														{{-- {!!
												...
												<x-bootstrap::column class="px-0">
													<x-bootstrap::media variant="primary" class="mt-4"
														title="<small>Deductions</small>" />
												</x-bootstrap::column>
												<div class="form-group row mb-0">
													<label class="col-sm-5 col-form-label">
														BPJS Kesehatan<br />
														<small class="text-600">Total dependents
												...
												<x-bootstrap::column class="px-0">
													<x-bootstrap::media variant="primary" class="mt-4"
														title="<small>Penghasilan Tidak Kena Pajak</small>" />
												</x-bootstrap::column>
												<div class="form-group row mb-0">
												...
												<x-bootstrap::column class="px-0">
													<x-bootstrap::media variant="primary" class="mt-4"
														title="<small>PPH {{ $payroll->result->taxable->liability->rule }}</small>" />
												</x-bootstrap::column>
												<div class="form-group row mb-0">
												...
												<x-bootstrap::column class="px-0">
													<x-bootstrap::media variant="primary" class="mt-4"
														title="<small>Presences</small>" />
												</x-bootstrap::column>
												<div class="form-group row mb-0">
													<label class="col-sm-5 col-form-label">Present</label>
													<div class="col-sm-7 col-form-label">
														{{ $payroll->employee->presences->workDays ?? 0 }} day(s)
													</div>
												</div>
												<div class="form-group row mb-0">
												...
												<div class="form-group row mb-0">
													...
													<div class="col-sm-7 col-form-label">
														{{ $payroll->employee->presences->travelDays ?? 0 }} day(s)
														{{-- {{ $payroll->employee->presences->business_trip ?? 0 }} day(s)
														--}}
													</div>
												</div>
												<div class="form-group row mb-0">
													...
													<div class="col-sm-7 col-form-label">
														{{ $payroll->employee->presences->permits ?? 0 }} day(s)
														{{-- {{ $payroll->employee->presences->permit ?? 0 }} day(s) --}}
													</div>
												</div>
												<div class="form-group row mb-0">
													<label class="col-sm-5 col-form-label">Absents</label>
													<div class="col-sm-7 col-form-label">
														{{ $payroll->employee->presences->absentDays ?? 0 }} day(s)
														{{-- {{ $payroll->employee->presences->absent ?? 0 }} day(s) --}}
													</div>
												</div>
